...
- **ORM/migrations:** Drizzle (schema-as-code close to the hand-written DDL style already used here) with `drizzle-kit` for migrations, or `node-pg-migrate` with the DDL in `docs/schema.sql` as the first migration verbatim — either is fine; recommend Drizzle for the type-safe query layer the read-heavy views above will benefit from.
