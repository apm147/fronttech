...
| 1 | Apply `docs/schema.sql` to a real Neon database; load the six-sector seed data; verify row counts and spot-check against the prototype; add one placeholder-status sector row to exercise the "Coming soon" pattern (see [Resolved decisions](#resolved-decisions)) — **done**. Schema and seed applied to the real Neon database (7 sectors incl. the `tbd-seventh` placeholder, 435 records, 23 badges, 60 record_badges, 435 record_history — matches the local validation run exactly). Applied by executing `prisma/migrations/0001_init/migration.sql` and `db/seed/001_migrated_records.sql` directly over Neon's WebSocket driver (`@neondatabase/serverless`, tunnels over port 443) rather than `npx prisma migrate deploy`/`db seed`, since this sandbox's egress proxy is HTTPS/CONNECT-only and raw Postgres wire protocol (port 5432) can't traverse it regardless of allowlisting — see `prisma.config.ts`'s header comment for why a JS-engine driver-adapter workaround for `migrate deploy` itself was tried and reverted (upstream Prisma bug). The `_prisma_migrations` bookkeeping row for `0001_init` was hand-inserted to match, so a later `prisma migrate deploy` from an environment with real TCP access (Render) sees it as already applied. `npx prisma generate` verified against the real installed Prisma CLI (6.19.3); fixed one schema issue found in the process (the `PolicySignalByCategory` view's `@@id` isn't valid for Prisma's `views` preview feature — needs `@@unique` instead, since views have no real primary key). **Any future migration (0002+) added from a sandboxed session without raw TCP access needs the same manual workaround**: apply the new migration's SQL directly via the Neon WebSocket driver, then hand-insert its `_prisma_migrations` row — don't try the JS-engine/adapter route for `migrate deploy` itself, it's confirmed broken for applying new migrations (reproduced against a throwaway test migration, not just the first one). |
| 2 | Next.js read-only app (landscape grid, sector view, tabs/sub-tabs, search) deployed to Render, replacing the static HTML prototype — public, no login — **done**. `app/` (App Router) + `src/lib/data.ts` (Prisma query layer) + `src/config/` (sectors.ts, sections.ts — presentation metadata ported 1:1 from the prototype's `SECTOR_META`/`BC_*`/`SECTIONS`, per [What's deliberately not in this schema](#whats-deliberately-not-in-this-schema)). Tabs/sub-tabs are plain `Link`s driven by `?section=&sub=` query params — no client JS needed, matching "server components for the read path." Cross-sector search (`/search`) does a case-insensitive `ILIKE` across title/subtitle/description — the concrete instance of "Cross-sector search and filtering" from [Features and functionality](#features-and-functionality). CSV import/export and the toast notifier from the prototype are deliberately not ported — those are Phase 3's editorial write path, not this read-only phase. Deployed to Render (build: `npm install && npm run build`, start: `npm start`), pulling from the real Neon database via `src/lib/prisma.ts`'s `pg`/`@prisma/adapter-pg` path over Render's normal TCP access — confirmed working end to end, real data loading in production. Two deploy snags hit and fixed along the way: (1) a fresh `npm install` never generated `@prisma/client`'s runtime code, since nothing invoked `prisma generate` — fixed with a `postinstall` script; (2) `DATABASE_URL` reaching the running app as empty/malformed produced an opaque `ECONNREFUSED` (`pg`'s `Pool` silently falls back to `localhost:5432` when `connectionString` is falsy) — fixed by having `src/lib/prisma.ts` throw a clear error when `DATABASE_URL` is unset, and by re-entering the env var in Render without the surrounding quotes that had come along from copy-pasting the `.env.example` format. |
