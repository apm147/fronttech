...
| 2 | Next.js read-only app (landscape grid, sector view, tabs/sub-tabs, search) deployed to Render, replacing the static HTML prototype — public, no login — **app built, not yet deployed**. `app/` (App Router) + `src/lib/data.ts` (Prisma query layer) + `src/config/` (sectors.ts, sections.ts — presentation metadata ported 1:1 from the prototype's `SECTOR_META`/`BC_*`/`SECTIONS`, per [What's deliberately not in this schema](#whats-deliberately-not-in-this-schema)). Tabs/sub-tabs are plain `Link`s driven by `?section=&sub=` query params — no client JS needed, matching "server components for the read path." Cross-sector search (`/search`) does a case-insensitive `ILIKE` across title/subtitle/description — the concrete instance of "Cross-sector search and filtering" from [Features and functionality](#features-and-functionality). Verified with a real Postgres 16 local instance seeded from `db/seed/001_migrated_records.sql` (this sandbox can't reach Neon over raw TCP any more than `prisma migrate deploy` could, per the Phase 1 row above) — visually checked all four page states (landscape grid incl. placeholder card, sector timeline, a regulation sub-tab, cross-sector search results) plus dark mode via Playwright screenshots, and `npm run build` passes clean. CSV import/export and the toast notifier from the prototype are deliberately not ported — those are Phase 3's editorial write path, not this read-only phase. **Still open for Phase 2:** actual Render deployment (needs the real `DATABASE_URL` set in Render's dashboard) and a post-deploy check that `pg`/`@prisma/adapter-pg` (`src/lib/prisma.ts`) reaches Neon fine over Render's real TCP access, since that path has only been exercised against local Postgres so far. |
